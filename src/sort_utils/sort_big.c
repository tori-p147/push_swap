...
/*   Updated: 2025/08/09 21:59:14 by vmatsuda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	process_b(t_all *all)
{
	...
	while (flag_nodes > 0)
	{
		...
		curr = all->stack_b->head;
		// ft_printf("NEXT = [%d] MID = %d all.flag =
			//%d curr = [%d] flag =%d next = %p\n", all->next, all->mid,
			//all->flag, curr->order,
		// 	curr->flag, curr->next);
...
void	sort_big(t_all *all)
{
			...
			all->flag))
	{
		...
		// 	ft_printf("current next = %d, flag = %d\n", all->next, all->flag);
		process_b(all);
		if (!stack_contains_flag(all->stack_b, all->flag)
			&& !stack_contains_flag(all->stack_a, all->flag))
			all->flag++;
		ptr_head_a = all->stack_a->head;
		// i++;
